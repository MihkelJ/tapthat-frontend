...
import type { PositiveResponse } from './client';
...
export async function smartVerificationFlow(
  ...
  const tapIds = taps.map(tap => tap.id).filter(Boolean) as string[];
...
export async function progressiveVerification(tapIds: string[], walletAddress: string): Promise<string[]> {
...
export function findCompatibleTaps(
  taps: PositiveResponse['get /v1/beer-taps']['data']['beerTaps'],
  targetTap: PositiveResponse['get /v1/beer-taps']['data']['beerTaps'][number]
): PositiveResponse['get /v1/beer-taps']['data']['beerTaps'] {
...
export function groupTapsByVerificationRequirements(
  taps: PositiveResponse['get /v1/beer-taps']['data']['beerTaps']
): Record<string, PositiveResponse['get /v1/beer-taps']['data']['beerTaps']> {
  const groups: Record<string, PositiveResponse['get /v1/beer-taps']['data']['beerTaps']> = {};
